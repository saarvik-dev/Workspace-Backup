https://leetcode.com/problems/search-in-rotated-sorted-array/

https://leetcode.com/problems/majority-element/description/

https://leetcode.com/problems/find-the-index-of-the-first-occurrence-in-a-string/

https://leetcode.com/problems/vertical-order-traversal-of-a-binary-tree/

https://leetcode.com/problems/delete-node-in-a-bst/

https://leetcode.com/problems/lowest-common-ancestor-of-a-binary-search-tree/

https://leetcode.com/problems/minimum-size-subarray-sum/

https://leetcode.com/problems/longest-subarray-of-1s-after-deleting-one-element/description/?envType=problem-list-v2&envId=sliding-window

https://leetcode.com/problems/fruit-into-baskets/?envType=problem-list-v2&envId=sliding-window

https://leetcode.com/problems/count-number-of-nice-subarrays/description/?envType=problem-list-v2&envId=sliding-window

https://leetcode.com/problems/find-the-duplicate-number/?envType=problem-list-v2&envId=two-pointers

https://leetcode.com/problems/permutations/

https://leetcode.com/problems/construct-binary-tree-from-preorder-and-inorder-traversal/?envType=problem-list-v2&envId=tree

https://leetcode.com/problems/course-schedule/description/

https://leetcode.com/problems/excel-sheet-column-number/

https://leetcode.com/problems/add-digits/

https://leetcode.com/problems/maximum-total-subarray-value-i/?envType=daily-question&envId=2026-06-09

https://leetcode.com/problems/burst-balloons/

https://leetcode.com/problems/maximum-width-of-binary-tree/description/

https://leetcode.com/problems/maximum-product-subarray/description/

https://leetcode.com/problems/reverse-nodes-in-k-group/description/

The BST Iterator class is very important and also used in future questions, as it can be used to traverse the tree with a space complexity of just `O(h)`

***VERY IMPORTANT***

https://leetcode.com/problems/maximum-sum-bst-in-binary-tree/

Merge Two sorted arrays without extra space

Do optimal solution for the below problem, quite lengthy

https://leetcode.com/problems/valid-subarrays-with-matching-sum-digits-i/submissions/2043019349/

https://leetcode.com/problems/valid-sudoku/description/

Check bit manipulation solution

https://leetcode.com/problems/sum-of-all-subset-xor-totals/submissions/2047144342/?envType=problem-list-v2&envId=backtracking

Cycle Solution has to be done

https://leetcode.com/problems/find-eventual-safe-states/?envType=problem-list-v2&envId=graph

Very non intuitive that we can use graphs here !!!

https://leetcode.com/problems/couples-holding-hands/description/?envType=problem-list-v2&envId=graph

Rerooting DP Technique

https://leetcode.com/problems/sum-of-distances-in-tree/description/?envType=problem-list-v2&envId=graph

https://leetcode.com/problems/find-the-safest-path-in-a-grid/?envType=daily-question&envId=2026-07-01

# Very Important


https://leetcode.com/problems/network-recovery-pathways/description/?envType=daily-question&envId=2026-07-03

https://leetcode.com/problems/maximum-number-of-points-with-cost/description/?envType=problem-list-v2&envId=dynamic-programming

Important Custom Comparator

https://leetcode.com/problems/top-k-frequent-words/?envType=problem-list-v2&envId=heap-priority-queue

https://leetcode.com/problems/different-ways-to-add-parentheses/?envType=problem-list-v2&envId=dynamic-programming

Something related to modulo multiplicative inverse

https://leetcode.com/problems/minimum-total-cost-to-process-all-elements/description/

Very Good Question for Dijkstra Practice

https://leetcode.com/problems/find-the-city-with-the-smallest-number-of-neighbors-at-a-threshold-distance/

Forces you to visualize the information as nodes and edges - DSU solution to be done, i have only done the brute force normal BFS / DFS Solution

https://leetcode.com/problems/evaluate-division/description/

Very Good Question

https://leetcode.com/problems/all-possible-full-binary-trees/description/?envType=problem-list-v2&envId=dynamic-programming

# New Patterns


https://leetcode.com/problems/continuous-subarray-sum/description/

https://leetcode.com/problems/longest-subsequence-with-non-zero-bitwise-xor/description/

## Redo

https://leetcode.com/problems/sum-of-subarray-minimums/description/

https://leetcode.com/problems/target-sum/description/

optimal solution not understood

https://leetcode.com/problems/2-keys-keyboard/?envType=problem-list-v2&envId=dynamic-programming

### Math Intensive 

https://leetcode.com/problems/maximum-value-at-a-given-index-in-a-bounded-array/

https://leetcode.com/problems/smallest-subsequence-of-distinct-characters/description/?envType=daily-question&envId=2026-07-19

number of wonderful substrings → very bizarre question, quite forgetful and confusing

## TOD using Union Find 

Redundant Connection

https://leetcode.com/problems/number-of-operations-to-make-network-connected/?envType=problem-list-v2&envId=graph

https://leetcode.com/problems/stone-game-ii/ - Tabulation Code

## https://leetcode.com/problems/stone-game-iv/

Easy question but is a very important pattern for game questions

Good non Inutuitive question

https://leetcode.com/problems/number-of-matching-subsequences/description/?envType=problem-list-v2&envId=dynamic-programming

### Important Questions

https://leetcode.com/problems/maximal-square/?envType=problem-list-v2&envId=dynamic-programming

## Graph Modelling Questions


| LC No. | Problem |
| --- | --- |
| 752 | Open the Lock |
| 127 | Word Ladder |
| 433 | Minimum Genetic Mutation |
| 399 | Evaluate Division |
| 909 | Snakes and Ladders |
| 815 | Bus Routes |



https://leetcode.com/problems/split-array-into-consecutive-subsequences/description/


---

🔗 **References**
- https://leetcode.com/problems/search-in-rotated-sorted-array/ → https://leetcode.com/problems/search-in-rotated-sorted-array/
- https://leetcode.com/problems/majority-element/description/ → https://leetcode.com/problems/majority-element/description/
- https://leetcode.com/problems/find-the-index-of-the-first-occurrence-in-a-string/ → https://leetcode.com/problems/find-the-index-of-the-first-occurrence-in-a-string/
- https://leetcode.com/problems/vertical-order-traversal-of-a-binary-tree/ → https://leetcode.com/problems/vertical-order-traversal-of-a-binary-tree/
- https://leetcode.com/problems/delete-node-in-a-bst/ → https://leetcode.com/problems/delete-node-in-a-bst/
- https://leetcode.com/problems/lowest-common-ancestor-of-a-binary-search-tree/ → https://leetcode.com/problems/lowest-common-ancestor-of-a-binary-search-tree/
- https://leetcode.com/problems/minimum-size-subarray-sum/ → https://leetcode.com/problems/minimum-size-subarray-sum/
- https://leetcode.com/problems/longest-subarray-of-1s-after-deleting-one-element/description/?envType=problem-list-v2&envId=sliding-window → https://leetcode.com/problems/longest-subarray-of-1s-after-deleting-one-element/description/?envType=problem-list-v2&envId=sliding-window
- https://leetcode.com/problems/fruit-into-baskets/?envType=problem-list-v2&envId=sliding-window → https://leetcode.com/problems/fruit-into-baskets/?envType=problem-list-v2&envId=sliding-window
- https://leetcode.com/problems/count-number-of-nice-subarrays/description/?envType=problem-list-v2&envId=sliding-window → https://leetcode.com/problems/count-number-of-nice-subarrays/description/?envType=problem-list-v2&envId=sliding-window
- https://leetcode.com/problems/find-the-duplicate-number/?envType=problem-list-v2&envId=two-pointers → https://leetcode.com/problems/find-the-duplicate-number/?envType=problem-list-v2&envId=two-pointers
- https://leetcode.com/problems/permutations/ → https://leetcode.com/problems/permutations/
- https://leetcode.com/problems/construct-binary-tree-from-preorder-and-inorder-traversal/?envType=problem-list-v2&envId=tree → https://leetcode.com/problems/construct-binary-tree-from-preorder-and-inorder-traversal/?envType=problem-list-v2&envId=tree
- https://leetcode.com/problems/course-schedule/description/ → https://leetcode.com/problems/course-schedule/description/
- https://leetcode.com/problems/excel-sheet-column-number/ → https://leetcode.com/problems/excel-sheet-column-number/
- https://leetcode.com/problems/add-digits/ → https://leetcode.com/problems/add-digits/
- https://leetcode.com/problems/maximum-total-subarray-value-i/?envType=daily-question&envId=2026-06-09 → https://leetcode.com/problems/maximum-total-subarray-value-i/?envType=daily-question&envId=2026-06-09
- https://leetcode.com/problems/burst-balloons/ → https://leetcode.com/problems/burst-balloons/
- https://leetcode.com/problems/maximum-width-of-binary-tree/description/ → https://leetcode.com/problems/maximum-width-of-binary-tree/description/
- https://leetcode.com/problems/maximum-product-subarray/description/ → https://leetcode.com/problems/maximum-product-subarray/description/
- https://leetcode.com/problems/reverse-nodes-in-k-group/description/ → https://leetcode.com/problems/reverse-nodes-in-k-group/description/
- https://leetcode.com/problems/maximum-sum-bst-in-binary-tree/ → https://leetcode.com/problems/maximum-sum-bst-in-binary-tree/
- https://leetcode.com/problems/valid-subarrays-with-matching-sum-digits-i/submissions/2043019349/ → https://leetcode.com/problems/valid-subarrays-with-matching-sum-digits-i/submissions/2043019349/
- https://leetcode.com/problems/valid-sudoku/description/ → https://leetcode.com/problems/valid-sudoku/description/
- https://leetcode.com/problems/sum-of-all-subset-xor-totals/submissions/2047144342/?envType=problem-list-v2&envId=backtracking → https://leetcode.com/problems/sum-of-all-subset-xor-totals/submissions/2047144342/?envType=problem-list-v2&envId=backtracking
- https://leetcode.com/problems/find-eventual-safe-states/?envType=problem-list-v2&envId=graph → https://leetcode.com/problems/find-eventual-safe-states/?envType=problem-list-v2&envId=graph
- https://leetcode.com/problems/couples-holding-hands/description/?envType=problem-list-v2&envId=graph → https://leetcode.com/problems/couples-holding-hands/description/?envType=problem-list-v2&envId=graph
- https://leetcode.com/problems/sum-of-distances-in-tree/description/?envType=problem-list-v2&envId=graph → https://leetcode.com/problems/sum-of-distances-in-tree/description/?envType=problem-list-v2&envId=graph
- https://leetcode.com/problems/find-the-safest-path-in-a-grid/?envType=daily-question&envId=2026-07-01 → https://leetcode.com/problems/find-the-safest-path-in-a-grid/?envType=daily-question&envId=2026-07-01
- https://leetcode.com/problems/network-recovery-pathways/description/?envType=daily-question&envId=2026-07-03 → https://leetcode.com/problems/network-recovery-pathways/description/?envType=daily-question&envId=2026-07-03
- https://leetcode.com/problems/maximum-number-of-points-with-cost/description/?envType=problem-list-v2&envId=dynamic-programming → https://leetcode.com/problems/maximum-number-of-points-with-cost/description/?envType=problem-list-v2&envId=dynamic-programming
- https://leetcode.com/problems/top-k-frequent-words/?envType=problem-list-v2&envId=heap-priority-queue → https://leetcode.com/problems/top-k-frequent-words/?envType=problem-list-v2&envId=heap-priority-queue
- https://leetcode.com/problems/different-ways-to-add-parentheses/?envType=problem-list-v2&envId=dynamic-programming → https://leetcode.com/problems/different-ways-to-add-parentheses/?envType=problem-list-v2&envId=dynamic-programming
- https://leetcode.com/problems/minimum-total-cost-to-process-all-elements/description/ → https://leetcode.com/problems/minimum-total-cost-to-process-all-elements/description/
- https://leetcode.com/problems/find-the-city-with-the-smallest-number-of-neighbors-at-a-threshold-distance/ → https://leetcode.com/problems/find-the-city-with-the-smallest-number-of-neighbors-at-a-threshold-distance/
- https://leetcode.com/problems/evaluate-division/description/ → https://leetcode.com/problems/evaluate-division/description/
- https://leetcode.com/problems/all-possible-full-binary-trees/description/?envType=problem-list-v2&envId=dynamic-programming → https://leetcode.com/problems/all-possible-full-binary-trees/description/?envType=problem-list-v2&envId=dynamic-programming
- https://leetcode.com/problems/continuous-subarray-sum/description/ → https://leetcode.com/problems/continuous-subarray-sum/description/
- https://leetcode.com/problems/longest-subsequence-with-non-zero-bitwise-xor/description/ → https://leetcode.com/problems/longest-subsequence-with-non-zero-bitwise-xor/description/
- https://leetcode.com/problems/sum-of-subarray-minimums/description/ → https://leetcode.com/problems/sum-of-subarray-minimums/description/
- https://leetcode.com/problems/target-sum/description/ → https://leetcode.com/problems/target-sum/description/
- https://leetcode.com/problems/2-keys-keyboard/?envType=problem-list-v2&envId=dynamic-programming → https://leetcode.com/problems/2-keys-keyboard/?envType=problem-list-v2&envId=dynamic-programming
- https://leetcode.com/problems/maximum-value-at-a-given-index-in-a-bounded-array/ → https://leetcode.com/problems/maximum-value-at-a-given-index-in-a-bounded-array/
- https://leetcode.com/problems/smallest-subsequence-of-distinct-characters/description/?envType=daily-question&envId=2026-07-19 → https://leetcode.com/problems/smallest-subsequence-of-distinct-characters/description/?envType=daily-question&envId=2026-07-19
- https://leetcode.com/problems/number-of-operations-to-make-network-connected/?envType=problem-list-v2&envId=graph → https://leetcode.com/problems/number-of-operations-to-make-network-connected/?envType=problem-list-v2&envId=graph
- https://leetcode.com/problems/stone-game-ii/ → https://leetcode.com/problems/stone-game-ii/
- https://leetcode.com/problems/stone-game-iv/ → https://leetcode.com/problems/stone-game-iv/
- https://leetcode.com/problems/number-of-matching-subsequences/description/?envType=problem-list-v2&envId=dynamic-programming → https://leetcode.com/problems/number-of-matching-subsequences/description/?envType=problem-list-v2&envId=dynamic-programming
- https://leetcode.com/problems/maximal-square/?envType=problem-list-v2&envId=dynamic-programming → https://leetcode.com/problems/maximal-square/?envType=problem-list-v2&envId=dynamic-programming
- https://leetcode.com/problems/split-array-into-consecutive-subsequences/description/ → https://leetcode.com/problems/split-array-into-consecutive-subsequences/description/

